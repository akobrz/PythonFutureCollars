...
class WeatherForecast:
	...
	def __getitem__(self, date2check):
		self.w = Weather((self.key, date2check))
		answer = None
		try:
			answer = self.w.prepare_answer()
		except self.w.my_exception:
			pass
		return answer

	def items(self):
		return (row for row in open(self.fn, "r"))

	def __iter__(self):
		return self

	def __next__(self):
		f = [row.split(",")[0] for row in open(self.fn, "r")]
		if self._index < len(f):
...
class Weather:
	...
	def weather_future(self, weather_description):
	...
	def weather_past(self, weather_description):
		if weather_description is None:
			return "Nie wiem"
		elif "rain" in weather_description:
			return "Padało"
	...
	def prepare_answer(self):
		if self.check_in_file():
			...
			return self.save_to_file(self.date2check, self.weather_past(j["current"]["weather"][0]["description"]))
		else:
			for day in self.get_forecast()["list"]:
				if self.convert_unix2date(day["dt"]) == self.date2check:
					return self.save_to_file(self.date2check, self.weather_future(day["weather"][0]["description"]))
			return self.save_to_file(self.date2check, "Nie wiadomo")
